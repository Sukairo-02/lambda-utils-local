...
function streamToSharp(width: number) {
    return sharp().toFormat('webp').resize(width);
}
...
export const main: S3Handler = async (event) => {
    ...
    const newKey = `${prefix}-${Key.split('.')[0]}.webp`;
    ...
    // Trigger the streams
    (readStream?.Body as NodeJS.ReadableStream).pipe(resizeStream).pipe(writeStream);
